...
import characterSheetBg from './img/characterSheet.png';
...
const CharacterSheetDocument = ({ characters }) => (
  <Document>
...
const ZeroLevelGenerator = () => {
  ...
  const generateCharacters = () => {
    const newCharacters = [];
    for (let i = 0; i < 4; i++) {
      newCharacters.push(generateRandomCharacter());
    }
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <h2>DCC Zero Level Character Generator</h2>
      
      <button onClick={generateCharacters} style={{ 
        padding: '10px 20px', 
        fontSize: '16px', 
        marginBottom: '20px',
        ...
        <div>
          ...
          {characters.map((char, index) => (
            <div key={index} style={{ marginBottom: '10px', padding: '10px', border: '1px solid #ccc' }}>
              <strong>{char.name}</strong> - {char.gender} {char.alignment} {char.occupation}
            </div>
          ))}
          
          <PDFDownloadLink
            document={<CharacterSheetDocument characters={characters} />}
            fileName="dcc-zero-level-characters.pdf"
            style={{
              textDecoration: 'none',
              padding: '10px 20px',
              color: 'white',
          ...
          >
            {({ blob, url, loading, error }) =>
              loading ? 'Loading document...' : 'Download Character Sheets PDF'
            }
          </PDFDownloadLink>
        </div>
      )}
...
export default ZeroLevelGenerator;